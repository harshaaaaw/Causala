"""Backbone: shared event bus + subsystem registry for SIMFORGE.

SIMFORGE is a standalone product. It communicates through this in-process bus
(loose coupling) and persists evidence through the trustcore Spine
...
class EventBus:
    ...
    def publish(self, event: ControlEvent) -> None:
        for handler in self._subs.get(event.subsystem, []):
            try:
                handler(event)
            except Exception as exc:  # noqa: BLE001  (intentional: bus isolation; logged with context)
                logging.getLogger("simforge.bus").error(
...
def register_subsystem(inst: Subsystem) -> Subsystem:
    """Idempotent registration of a subsystem into the bus."""
